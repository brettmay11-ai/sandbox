...
http.createServer = function createServerWithTeacherDashboard(listener) {
  return originalCreateServer(async (request, response) => {
    try {
      ...
      if ((CLEAN_STUDENT_PAGES.has(pathname.slice(1)) || pathname === '/' || pathname === '/index.html') && request.method === 'GET' && parseCookies(request).nfl_admin_session) {
        const user = await getSessionUser(request);
