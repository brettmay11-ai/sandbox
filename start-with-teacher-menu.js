...
function serveStudentPortal(response) {
  ...
  const banner = '<div id="student-impersonation-banner" role="status" style="position:fixed;top:0;left:0;right:0;z-index:2147483647;min-height:52px;padding:10px 18px;background:#172554;color:#dbeafe;border-bottom:1px solid rgba(96,165,250,.65);font:700 12px Inter,system-ui,sans-serif;display:flex;align-items:center;justify-content:center;gap:12px;text-align:center;box-shadow:0 8px 24px rgba(0,0,0,.3)">Viewing as a student <button id="stop-student-impersonation" style="border:1px solid rgba(147,197,253,.55);border-radius:6px;padding:6px 10px;background:rgba(255,255,255,.12);color:#fff;font:800 11px Inter,system-ui,sans-serif;cursor:pointer">Return to Super Admin</button></div>';
  html = html.replace(/<body([^>]*)>/i, '<body$1 style="padding-top:52px !important">' + banner);
