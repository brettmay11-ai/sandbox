...
function addAccountControls() {const nav = document.querySelector('nav');const right = nav?.lastElementChild;if (!right || document.getElementById('account-controls')) return;const controls = document.createElement('div');controls.id = 'account-controls';controls.className = 'shrink-0 flex items-center gap-2';const initials=escapeHtml(initialsFor(portalUser.displayName)),safeName=escapeHtml(portalUser.displayName||'Student');const teacherPortalButton=portalUser.role === 'teacher' ? '<a href="/teacher" title="Return to teacher portal" class="hidden sm:inline-flex items-center gap-2 min-h-10 px-4 rounded-lg border border-sky-400/35 bg-sky-400/12 text-sky-100 text-xs font-black uppercase tracking-wide hover:bg-sky-400/20 hover:border-sky-300/60 transition"><iconify-icon icon="lucide:arrow-left-to-line" class="text-base"></iconify-icon><span>Teacher Portal</span></a><a href="/teacher" title="Return to teacher portal" aria-label="Return to teacher portal" class="sm:hidden w-10 h-10 rounded-lg border border-sky-400/35 bg-sky-400/12 text-sky-100 grid place-items-center"><iconify-icon icon="lucide:arrow-left-to-line" class="text-lg"></iconify-icon></a>' : '';controls.innerHTML = `${teacherPortalButton}<a id="portal-profile" href="${portalUser.role==='student'?'/profile':'/teacher/dashboard'}" title="${portalUser.role==='student'?'Open profile':'Teacher dashboard'}" aria-label="${portalUser.role==='student'?'Open profile':'Teacher dashboard'} for ${safeName}" class="student-team-mark w-9 h-9 rounded-full grid place-items-center text-[11px] font-black tracking-wide">${initials}</a><button id="portal-logout" title="Sign out" class="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center hover:bg-red-500/15 hover:text-red-300"><iconify-icon icon="lucide:log-out"></iconify-icon></button>`;right.appendChild(controls);controls.querySelector('#portal-logout').addEventListener('click', async () => { await portalApi('/api/logout', { method:'POST' }); window.location.href='/login'; });}
async function initializeAccountExperience() {try {const { user } = await portalApi('/api/me');portalUser = user;applyAssignedTeamBranding(); addAccountControls(); showTeamTunnelEntrance();} catch (error) { console.warn('Account experience could not initialize.', error); }}
