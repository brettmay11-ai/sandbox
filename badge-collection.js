(() => {
  ...
  function attachDashboardPreview(profile) {
    const attach = () => {
      ...
      const html = `<div class="p-5 border-b border-white/10 flex items-center justify-between gap-3"><div><div class="text-[10px] uppercase text-white/35 font-bold">Badge Collection</div><h2 class="text-xl font-black mt-1">${profile.earnedCount} of ${profile.total} earned</h2></div><a href="/profile" class="px-4 py-2 text-xs font-bold student-team-mark">Open Profile</a></div><div class="grid md:grid-cols-3 gap-px bg-white/10">${earned.length ? earned.map(badge => `<div class="bg-[#111]">${badgeMarkup(badge, true)}</div>`).join('') : '<div class="md:col-span-3 bg-[#111] p-6 text-sm text-white/35">Earn your first badge by answering a math play, submitting writing, or completing a City Scout challenge.</div>'}</div>`;
