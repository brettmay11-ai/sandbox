(() => {
  function initialize() {
    ...
    function difficultyLabel(boost) {
      return ['Normal','Harder','Advanced','Challenge'][Math.max(0, Math.min(3, Number(boost) || 0))] || 'Normal';
    }

    function openModal(studentId, displayName, username, mathDifficultyBoost = 0) {
      closeModal();
      ...
      modal.innerHTML = `<div role="dialog" aria-modal="true" aria-labelledby="edit-student-title" class="w-full max-w-md rounded-2xl border border-white/10 bg-slate-950 p-6 shadow-2xl">
        ...
        <div class="mt-6 space-y-4"><label class="block text-xs text-white/55">Display name<input data-edit-display maxlength="80" class="${inputClass}" /></label><label class="block text-xs text-white/55">Username<input data-edit-username maxlength="32" class="${inputClass}" /></label><label class="block text-xs text-white/55">Math questions<select data-edit-math-boost class="${inputClass}"><option value="0">Normal</option><option value="1">Harder</option><option value="2">Advanced</option><option value="3">Challenge mode</option></select><span class="mt-1 block text-[10px] text-white/35">Boosts question difficulty only. XP level and rewards stay the same.</span></label></div>
      ...
      const boostInput = modal.querySelector('[data-edit-math-boost]');
      const status = modal.querySelector('[data-edit-status]');
      displayInput.value = displayName; usernameInput.value = username;
      boostInput.value = String(Math.max(0, Math.min(3, Number(mathDifficultyBoost) || 0)));
      ...
      modal.querySelector('[data-save-edit]').addEventListener('click', async event => {
        ...
        try {
          const response = await fetch(`/api/teacher/students/${studentId}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({displayName:displayInput.value, username:usernameInput.value, mathDifficultyBoost:Number(boostInput.value)}) });
          ...
          if (row) row.dataset.mathDifficultyBoost = String(data.student.math_difficulty_boost || 0);
          const boostLabel = document.querySelector(`[data-student-math-boost="${studentId}"]`);
          if (boostLabel) boostLabel.textContent = difficultyLabel(data.student.math_difficulty_boost);
    ...
    function enhanceRows() {
      tbody.querySelectorAll('tr').forEach(row => {
        ...
        editButton.addEventListener('click', () => openModal(match[1], name.textContent.trim(), username.textContent.trim(), row.dataset.mathDifficultyBoost));
