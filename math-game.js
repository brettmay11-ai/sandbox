(() => {
  ...
  style.textContent = `
    ...
    .math-game-midfield-logo{position:absolute;left:50%;top:50%;z-index:4;width:62px;height:62px;padding:7px;display:grid;place-items:center;border:2px solid rgba(255,255,255,.7);border-radius:50%;background:rgba(255,255,255,.84);opacity:.9;transform:translate(-50%,-50%);filter:drop-shadow(0 3px 4px rgba(0,0,0,.62));transition:opacity .2s ease,filter .2s ease}.math-game-midfield-logo.is-loaded{opacity:.94}
  ...
  async function renderAssignedTeamMark() {
    try {
      const data=await api('/api/me',{cache:'no-store'}), teamKey=data.user?.selectedTeam||data.selectedTeam, team=typeof getNFLTeamBrand==='function'?getNFLTeamBrand(teamKey):null;
