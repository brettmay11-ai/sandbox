  ...
  http.createServer=function classroomGameServer(originalListener){
    return createServer(async(request,response)=>{
      try{
        ...
        if(pathname.startsWith('/api/math-game/')){
          const user=await getUser(request);
          await handleMathGame({pool,req:request,res:response,path:pathname,user,sendJson,readJson});
          return;
        }
        originalListener(request,response);
      }catch(error){console.error('Math game request failed.',error);if(!response.headersSent)sendJson(response,500,{error:'Game server error.'});else response.end();}
    });
  };
  require('./server');
})().catch(error=>{console.error('Math game initialization failed.',error);process.exit(1)});
