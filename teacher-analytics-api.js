async function handleTeacherAnalytics({pool,req,res,path,user,sendJson}){
  ...
  sendJson(res,200,{students,summary:{activeStudents:active.length,totalXp:sum('total_xp'),weeklyXp:sum('weekly_xp'),writingXp:sum('writing_xp'),writingSubmissions:sum('writing_submissions'),averageAccuracy:average('accuracy'),totalAnswers:sum('questions_answered'),totalMinutes:Math.round(sum('time_seconds')/60)}});return true
}
