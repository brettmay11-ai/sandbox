...
async function handleMathGame({pool,req,res,path,user,sendJson,readJson}){
  ...
  if(path==='/api/math-game/challenge'&&req.method==='POST'){
    ...
    const profile=(await pool.query('SELECT total_xp FROM math_profiles WHERE user_id=$1',[user.id])).rows[0];
    const challenge=createQuestion(body.yards,Number(profile?.total_xp||0)),id=crypto.randomBytes(24).toString('hex');
