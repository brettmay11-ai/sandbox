...
function createQuestion() {
  ...
  if(type===4){const distance=pick([500,750,1000,1250,1500,1750,2000,2250,2500]);const minutes=Math.round(distance/500*60);return {question:`The ${a} travel ${distance.toLocaleString()} miles at 500 miles per hour. About how many minutes is the flight?`,answer:minutes,explanation:`${distance.toLocaleString()} ÷ 500 = ${distance/500} hours. ${distance/500} × 60 = ${minutes} minutes.`,difficulty:'Captain',xp:20,yards:20};}
  if(type===5){const attempts=pick([20,25,40,50]),percent=pick([50,60,70,80]),complete=attempts*percent/100;return {question:`The ${a} quarterback completed ${complete} of ${attempts} passes. What was the completion percentage? Enter the number without the percent sign.`,answer:percent,explanation:`${complete} ÷ ${attempts} × 100 = ${percent}%`,difficulty:'Captain',xp:20,yards:20};}
  const capacity=pick([60000,65000,70000,75000,80000]),percent=pick([50,60,75,80,90]),attendance=capacity*percent/100;return {question:`A stadium holds ${capacity.toLocaleString()} fans. If ${attendance.toLocaleString()} attend, what percent of the stadium is full? Enter the number without the percent sign.`,answer:percent,explanation:`${attendance.toLocaleString()} ÷ ${capacity.toLocaleString()} × 100 = ${percent}%`,difficulty:'All-Pro',xp:30,yards:30};
}
