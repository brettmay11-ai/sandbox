...
function createTCUTravelSection(season) {
  ...
  tracker.className = 'max-w-7xl mx-auto mt-16 pt-12 border-t border-white/10';
  tracker.innerHTML = `<div class="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-8"><div><span class="text-[10px] font-mono tcu-purple uppercase tracking-widest mb-2 block">Distance & Geography</span><h3 class="text-3xl font-black tracking-tight">TCU Travel Tracker</h3><p class="text-sm text-white/45 mt-2">Follow the Horned Frogs from Fort Worth to every road game.</p></div><span class="text-[10px] font-mono text-white/35">${season} Season · Live Schedule</span></div><div id="tcu-travel-summary" class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6"></div><div class="grid grid-cols-1 lg:grid-cols-3 gap-4"><div class="lg:col-span-2 tcu-panel overflow-hidden"><div class="relative w-full min-h-[420px]" id="tcu-map-wrap"><svg id="tcu-travel-map" viewBox="0 0 960 600" class="w-full h-full min-h-[420px]"><rect width="960" height="600" fill="#08050d"/><g id="tcu-map-states"></g><g id="tcu-map-routes"></g><g id="tcu-map-points"></g></svg><div id="tcu-map-tooltip" class="map-tooltip"></div></div></div><div class="tcu-panel p-5"><h4 class="text-sm font-black mb-1 flex items-center gap-2"><iconify-icon icon="lucide:plane" class="tcu-purple"></iconify-icon> Road Games</h4><p class="text-[10px] text-white/35 mb-4">Distances from Fort Worth</p><div id="tcu-travel-list" class="space-y-2 max-h-[520px] overflow-y-auto pr-1"></div></div></div>`;
...
function renderTCUTravelSummary(destinations) {
  ...
  document.getElementById('tcu-travel-summary').innerHTML = [['Road Trips', destinations.length], ['Season Travel', `${Math.round(total).toLocaleString()} mi`], ['Average Trip', `${destinations.length ? Math.round(total / destinations.length).toLocaleString() : 0} mi`], ['Farthest Trip', farthest ? `${farthest.city}, ${farthest.state}` : '-']].map(([label, value]) => `<div class="tcu-stat-card p-4"><div class="text-[9px] text-white/35 uppercase tracking-wider font-black">${label}</div><div class="text-lg font-black mt-1">${value}</div></div>`).join('');
  document.getElementById('tcu-travel-list').innerHTML = destinations.map(destination => `<div class="tcu-row p-3"><div class="flex items-center justify-between gap-2"><span class="text-[11px] font-semibold truncate">${destination.neutral ? 'vs' : 'at'} ${destination.opponent}</span><span class="text-[11px] font-mono tcu-purple shrink-0">${Math.round(destination.distance).toLocaleString()} mi</span></div><div class="text-[9px] text-white/35 mt-1">Week ${destination.week} · ${destination.stadium}</div><div class="text-[9px] text-white/35">${destination.city}, ${destination.state}${destination.channel ? ` · ${destination.channel}` : ''}</div></div>`).join('');
}
...
function drawTCUDomesticDestination(routes, points, home, target, destination, index) {
  ...
  routes.append('line').attr('x1', home[0]).attr('y1', home[1]).attr('x2', target[0]).attr('y2', target[1]).attr('stroke', 'rgba(184,156,255,0.74)').attr('stroke-width', 2).attr('stroke-linecap', 'round').attr('stroke-dasharray', length).attr('stroke-dashoffset', length).style('animation', `draw-line 1.2s ease ${index * .08}s forwards`);
...
function drawTCUInternationalDestination(routes, points, home, projection, destination) {
  ...
  routes.append('line').attr('x1', home[0]).attr('y1', home[1]).attr('x2', home[0] + vx / magnitude * 1800).attr('y2', home[1] + vy / magnitude * 1800).attr('stroke', 'rgba(184,156,255,0.82)').attr('stroke-width', 2.5).attr('stroke-dasharray', '8 6');
...
async function renderTCUTravelMap(destinations) {
  ...
  svg.select('#tcu-map-states').selectAll('path').data(states.features).enter().append('path').attr('class', 'state-path').attr('d', path).attr('fill', 'rgba(255,255,255,.07)').attr('stroke', 'rgba(255,255,255,.18)');
  ...
  destinations.forEach((destination, index) => {
    const target = projection([destination.lng, destination.lat]);
    if (target) drawTCUDomesticDestination(routes, points, home, target, destination, index);
    else drawTCUInternationalDestination(routes, points, home, projection, destination);
  });
  const homePoint = points.append('g').attr('transform', `translate(${home[0]},${home[1]})`);
  homePoint.append('circle').attr('r', 10).attr('fill', '#4D1979').attr('stroke', '#fff').attr('stroke-width', 3);
  homePoint.append('text').attr('y', -16).attr('text-anchor', 'middle').attr('fill', '#fff').attr('font-size', '9px').attr('font-weight', '800').text('TCU');
}
...
function showTCUTravelTooltip(event, destination) {
  ...
  tooltip.innerHTML = `<div class="text-xs font-bold tcu-purple">${destination.abbr}: ${destination.opponent}</div><div class="text-[10px] text-white/50 mt-1">${destination.stadium}</div><div class="text-[10px] text-white/40">${destination.city}, ${destination.state}</div><div class="text-[10px] font-mono tcu-purple mt-2">${Math.round(destination.distance).toLocaleString()} miles from TCU</div>`;
  tooltip.style.left = `${Math.min(event.clientX - rect.left + 14, rect.width - 200)}px`;
  tooltip.style.top = `${Math.max(event.clientY - rect.top - 20, 10)}px`;
...
async function initializeTCUTravel() {
  ...
  try {
    ...
    const destinations = getTCUTravelDestinations(games.filter(game => game.HomeTeam === 'TCU' || game.AwayTeam === 'TCU'));
    createTCUTravelSection(season);
    renderTCUTravelSummary(destinations);
    await renderTCUTravelMap(destinations);
  } catch (error) {
    console.warn('TCU travel tracker could not be loaded.', error);
  }
}
